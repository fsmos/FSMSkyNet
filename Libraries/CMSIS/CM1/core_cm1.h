...
static __INLINE void NVIC_SetPriority ( IRQn_Type IRQn, uint32_t priority )
{
  if(IRQn < 0) {
    ...
    //NVIC ->IP[(uint32_t) (IRQn) >> 2] = ((priority << (31 - __NVIC_PRIO_BITS))>> ((31 - __NVIC_PRIO_BITS))) << (((IRQn & 0x03) << 3) + 6); /* set Priority for device specific Interrupts  */
		...
		NVIC ->IP[(uint32_t) (IRQn) >> 2] = (NVIC ->IP[(uint32_t) (IRQn) >> 2]&(~(0xC0<<((IRQn & 0x03) << 3)))) | ((priority&3) << (((IRQn & 0x03) << 3) + 6));
  }
}
...
static __INLINE uint32_t NVIC_GetPriority(IRQn_Type IRQn)
{
  ...
  if(IRQn < 0) {
    ...
    return((uint32_t)(NVIC->IP[(uint32_t)(IRQn)>>2] >> (((IRQn & 0x03) << 3) + 6))&3);      /* get priority for device specific interrupts  */
  }
}
